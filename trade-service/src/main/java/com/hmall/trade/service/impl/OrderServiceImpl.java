...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements IOrderService {
    ...
    public Long createOrder(OrderFormDTO orderFormDTO) {
        ...
        try {
            ...
            log.error("发送清理购物车商品消息失败,itemIds:{}", itemIds, e);
        }
        // 发送订单延迟消息, 检测订单状态
        try {
    ...
    public void cancelOrder(Long orderId) {
        // 标记订单状态为过期
        lambdaUpdate().set(Order::getStatus, 5).eq(Order::getId, orderId);

        // 返还商品库存
        List<OrderDetailDTO> detailDTOByOrderId = orderDetailService.getDetailDTOByOrderId(orderId);
        itemClient.restoreStock(detailDTOByOrderId);
    }
